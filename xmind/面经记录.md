...
3. HTTPS为什么安全？✅️（TLS握手，非对称加密-》预主密钥，对称加密-》主密钥，**具体流程**）
4. HTTP1.1、HTTP2.0、HTTP3.0✅️（1.1长连接，2.0多路复用，3.0QUIC，UDP+TLS）
5. WebRTC技术❌（实时音视频技术：媒体采集，信令交换、建立连接、安全传输）
6. 讲一下MySQL索引✅️（聚簇、非聚簇，B+树，磁盘IO，回表，**索引优化**，**索引失效**）
7. Redis常见数据结构✅️（String，List，Hash，Set，Zset，GEO，Stream，Bitmap，HyperLogLog，**RedisObject**（类型，编码，lru，refcount，ptr，对应函数））
8. Redis的zset使用场景✅️（实时排行榜：游戏，商品）
9. 讲一下消息队列的使用✅️（异步，解耦，削峰，eg：电商）
10. Go项目：具体的实现✅️（websocket管理，上下线，单聊群聊消息转发）
11. Go项目：如何实现消息已读和未读？✅️（收到ack，添加字段，上线读消息ack/修改字段）
12. Go项目：实际生产环境中多个服务器，消息如何确定接收方的位置转发的？✅️（Redis 映射 + Kafka 转发 + 服务发现）

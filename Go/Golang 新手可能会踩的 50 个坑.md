译文：[Golang 新手可能会踩的 50 个坑](https://segmentfault.com/a/1190000013739000)
原文：[50 Shades of Go: Traps, Gotchas, and Common Mistakes](https://golang50shades.com/)
**转载自译文链接**，并增加了一些注释（代码解释、Go新版本的变化）。文章分为三部分：初级篇 1-34，中级篇 35-50，高级篇 51-57。
...
可使用 [vet](https://link.segmentfault.com/?enc=N8IyAAwQOIS25og2uN%2FBdA%3D%3D.twZA0XHR2m%2BAsE4ob%2FlGcE0NxP304maq1UZNqZ36M6Y9yTj%2Bupx%2Bl0WHXQEtoSWE) 工具来诊断这种变量覆盖，Go 默认不做覆盖检查，添加 `-shadow` 选项来启用：
...
注意 vet 不会报告全部被覆盖的变量，可以使用 [go-nyet](https://link.segmentfault.com/?enc=WwzC4Zksxj9hpitqpeRutg%3D%3D.Y4K4fctKew9Rg%2FmOpkv%2Fb7mkpTjO5CBQg5ytA7PgySlfRav3IuRznXOktTasxdig) 来做进一步的检测：
...
[go-how-is-two-dimensional-arrays-memory-representation](https://link.segmentfault.com/?enc=6yRwNhOl2vBr5n8PGW9Prw%3D%3D.oaCY5VNjfAoEp3hUIIDeSGgLWmdkPt7bdHfemgGdTelm%2FaALmLEygH0b7zLpLJ%2FmP6RT581L7FsHlMm4C%2B0HDIASZDdgTikLxrfi9VFr7KJA3H9WMlhs8DJysyHN9x8Q2SeX7WdgDIwUwJ2ccKku9g%3D%3D)

[what-is-a-concise-way-to-create-a-2d-slice-in-go](https://link.segmentfault.com/?enc=7416zU8AgQee3KYKWeYhoQ%3D%3D.kpJfKIxadkw7IPtTvzHxfVu4n7Y2Agx%2F%2FEA0gDQjHBXBHu1iypD%2B3DG2AtjIy5gcIgRAeOdE2CVVBpuWH33k6P9vBrhjFrJDtJSnaNv3Q6N5Sw61g1XAKv9tpJhV13DE)
...
雾：[参考原文](https://link.segmentfault.com/?enc=bHVBny0svaqqrTveW5ftUg%3D%3D.uz%2BcWgcqZ%2BaFM8CrzMrRuT%2FmpVd%2FExYS8is5p5C3R6J8hWk4ByJMLqw8etbUqBi3pHu4u1khmUGrTozgVk5TLB4zumtj4kxb3CyNtaMbjaj6nLUiVxpGQoiCYMiusc1aHbCcR29m29R5gb0FrSlMGg%3D%3D)
...
如果需要使用 `for range` 迭代访问字符串中的字符（unicode code point / rune），标准库中有 `"unicode/utf8"` 包来做 UTF8 的相关解码编码。另外 [utf8string](https://link.segmentfault.com/?enc=RVDRzV97GISpXeI5DjYbJw%3D%3D.1zRhVK22LUx0OMpcGdXGaps5my6SRPRtHyd78pgzitEreRe%2FJ6AxkACMjZK06wM0) 也有像 `func (s *String) At(i int) rune` 等很方便的库函数。
...
参考：[normalization](https://link.segmentfault.com/?enc=BcwPPWF1RwSuLGfINh7UUg%3D%3D.NjwghDp%2B%2F4LV87A%2B8CE8ZwnJ%2FJyWflkkXRAcJvQpI%2FckqCrVeM%2FioVf6smb99N%2Bp)
...
注意一个字符可能占多个 rune，比如法文单词 café 中的 é。操作特殊字符可使用[norm](https://link.segmentfault.com/?enc=HYT%2FH1moJVqlVMZ8v0CQPg%3D%3D.wY%2FGKuD8Y5ldBsCSQtdKbpdE1eMu1RgewaE51z%2F4AkKEWWHPMPSdIHVJChR5KHzsEjRdreE2lnJ6Ym1pfEOI0w%3D%3D) 包。
...
如果你去 [Go Playground](https://link.segmentfault.com/?enc=tilACGN9237MajadTJzIVg%3D%3D.cDPzSzF9uMarnDD%2Fb8UrQVFSdM3s4eyULqws8jKGhds%3D) 重复运行上边的代码，输出是不会变的，只有你更新代码它才会重新编译。重新编译后迭代顺序是被打乱的：

![img](https://segmentfault.com/img/remote/1460000013739005?w=267&h=237)
...
![img](https://segmentfault.com/img/remote/1460000013739006?w=269&h=124)
...
![img](https://segmentfault.com/img/remote/1460000013739007?w=464&h=373)
...
![img](https://segmentfault.com/img/remote/1460000013739008?w=228&h=212)
...
![img](https://segmentfault.com/img/remote/1460000013739009?w=214&h=77)
...
![img](https://segmentfault.com/img/remote/1460000013739010?w=412&h=237)
...
![img](https://segmentfault.com/img/remote/1460000013739011?w=230&h=147)
...
![img](https://segmentfault.com/img/remote/1460000013739012)

> **代码执行流程**（Step-by-Step）
>
> 为了更清晰地理解，我们假设程序按以下时间线运行（`T0, T1, T2...`）：
>
> **T0: 初始化**
>
> - 主协程 (`main`) 创建了 `inCh`和 `outCh`（无缓冲通道）。
>
> - 主协程启动了两个匿名协程（`G1`：状态机，`G2`：输出消费者）。
>
> - `G1`(状态机) 开始执行，初始状态：
>
>   ```go
...
>   ```
>
> - `G2`(消费者) 开始执行，并在 `for range outCh`处阻塞，等待 `outCh`被关闭或有数据送来。
>
> - 主协程执行 `time.Sleep(0)`，让出CPU，其他协程得以运行。
>
> **T1: 状态机进入循环**
>
> - `G1`进入 `for`循环的 `select`语句。
> - 此时只有 `case val = <-in:`是可能准备好的（因为 `out`是 `nil`，`case out <- val:`被禁用）。
> - `G1`在 `case val = <-in:`上阻塞，等待 `inCh`被发送数据。
>
> **T2: 主协程发送数据 `1`**
>
> - 主协程执行 `inCh <- 1`。
>
> - 这个操作成功，因为 `G1`正在另一边等待接收。
>
> - `G1`的 `case val = <-in:`被选中，`val`被赋值为 `1`。
>
> - `G1`打印 `"++++++++++"`（表示执行了接收分支）。
>
> - `G1`执行分支内的代码，**改变状态**：
...
>   ```
>
> - 状态机从 **接收状态 (State A)** 切换到了 **发送状态 (State B)**。
>
> - `G1`开始下一次循环。
>
> **T3: 状态机尝试发送**
...
> - `G1`进入下一次 `select`。
>
> - 此时只有 `case out <- val:`是可能准备好的（因为 `in`是 `nil`，`case val = <-in:`被禁用）。
>
> - `val`的值现在是 `1`。
>
> - `G1`执行 `case out <- val:`，将 `1`发送到 `outCh`。
>
> - 这个操作成功，因为 `G2`(消费者) 一直在另一边等待接收。
>
> - `G1`打印 `"--------"`（表示执行了发送分支）。
>
> - `G1`执行分支内的代码，**再次改变状态**：
...
>   ```
>
> - 状态机从 **发送状态 (State B)** 切换回了 **接收状态 (State A)**。
>
> - `G2`(消费者) 接收到 `outCh`的值 `1`，打印 `"Result: 1"`。
>
> - `G1`开始下一次循环，并在 `case val = <-in:`上阻塞，等待下一个输入。
>
> **T4: 主协程发送数据 `2`**
>
> - 主协程执行 `inCh <- 2`。
...
> - 流程与 **T3** 完全相同。
> - `G1`发送 `2`，打印 `"--------"`，切换回接收状态。
> - `G2`接收到 `2`，打印 `"Result: 2"`。
>
> **T6: 等待与结束**
>
> - `G1`再次在 `case val = <-in:`上阻塞，等待永远不会到来的第三个输入。
> - 主协程 `Sleep 3秒`后结束，所有协程（包括 `G1`和 `G2`）被终止。
>
> **流程总结与输出结果**
> 整个程序的执行流程和输出顺序是严格确定的（原图应该是错的，我运行的结果如下）：
>
> ```
> 1.++++++++++(状态机接收到 1)
> 
...
![img](https://segmentfault.com/img/remote/1460000013739013?w=425&h=129)
...
> Get [https://api.ipify.org?format=...](https://link.segmentfault.com/?enc=lOLEBzWGJHPiDIVahAEu1g%3D%3D.kjq7QMLxY2SoXXXE2orzgqp7semnkR%2BTzp8ldSxjdtr%2F6aIeUg%2Fp9KwqpWuE9LlW): x509: certificate signed by unknown authority
...
如果 byte slice 中含有验证用户身份的数据（密文哈希、token 等），不应再使用 `reflect.DeepEqual()`、`bytes.Equal()`、 `bytes.Compare()`。这三个函数容易对程序造成 [timing attacks](https://link.segmentfault.com/?enc=u6p3TSXxr5F%2FlnUyrNRLPA%3D%3D.AT5EwyjMeZFkmLEb%2BhaRUK0CK5AYw1gVc07ebF0oOx9EpgTM3NZVijW2goKCDbkS)，此时应使用 "crypto/subtle" 包中的 `subtle.ConstantTimeCompare()` 等函数
...
> Go 新版本（尤其是 Go 1.22+）对 **for 循环中的迭代变量与闭包函数** 的行为进行了重要改进，解决了长期存在的变量捕获问题。以下是详细分析：
>
> ------
>
> **Go 1.22 之前的版本（传统行为）**
>
> 在 Go 1.21 及更早版本中，`for`循环的迭代变量 (`v`) 在每次迭代中会被 **重用**（即所有闭包共享同一个变量地址）。
>
> 这会导致以下经典问题：
>
> **问题代码示例**
...
>     for _, v := range data {
>         go func() {
>             fmt.Println(v) // 所有 goroutine 共享同一个 v 的地址！
>         }()
>     }
>     time.Sleep(time.Second)
>     // 输出：three three three（不符合预期）
> }
> ```
>
> **原因**：
>
> 所有 goroutine 启动时捕获的是同一个 `v`的引用，而 `v`在循环结束后最终值为 `"three"`，因此所有 goroutine 打印的都是 `"three"`。
>
> **传统解决方案**
>
> 1. **局部变量拷贝**
>
>    在循环内创建局部变量，切断闭包对迭代变量的依赖：
>
>    ```go
>    for _, v := range data {
>        v := v // 创建局部副本
...
>    ```go
>    for _, v := range data {
>        go func(in string) { fmt.Println(in) }(v)
>    }
>    ```
>
> ------
...
> **Go 1.22+ 的改进**
>
> 从 **Go 1.22 开始**，`for`循环的迭代变量 (`v`) 在每次迭代中会 **自动创建新的局部变量**，不再共享同一个内存地址。
>
> 这意味着闭包函数捕获的是当前迭代的独立值，无需手动拷贝！
>
> **新版本行为示例**
>
> ```go
...
>     for _, v := range data {
>         go func() {
>             fmt.Println(v) // 每个 v 是独立的！
>         }()
>     }
>     time.Sleep(time.Second)
>     // 输出（随机顺序）：one two three（符合预期）
> }
> ```
>
> **关键变化**：
>
> - 每次迭代的 `v`是全新的变量，闭包捕获的是当前迭代的瞬时值。
> - 不再需要 `v := v`或参数传递的 workaround。
>
> ------
>
> **对新旧代码的影响**
...
> | :---------------------- | :--------------------------- | :--------------------------------- |
> | 直接捕获迭代变量 `v`    | 所有闭包共享 `v`，输出错误值 | 每个闭包捕获独立的 `v`，输出正确值 |
> | 使用 `v := v`或参数传递 | 必须使用，否则出错           | 仍可使用，但不再必要               |
> | 向后兼容性              | 旧代码需手动修复             | 旧代码可能意外正确（需检查逻辑）   |
>
> #### **注意事项**
>
> 1. 旧代码迁移**：
>
>    如果旧代码依赖“共享迭代变量”的特性（罕见情况），升级到 Go 1.22+ 后可能需要调整逻辑。
>
> 2. **显式拷贝仍有效**：
>
>    即使在新版本中，显式使用 `v := v`或参数传递仍是安全的，且能明确表达意图。
>
> 3. **性能影响**：
>
>    新版本的实现可能会略微增加内存分配（每次迭代创建新变量），但通常可忽略不计。
>
> ------
>
> **复杂场景对比（结构体与方法接收者）**
...
> ```
>
> **原因**：`v.print()`的接收者是 `&v`，所有 goroutine 共享同一个 `v`的地址。
>
> **修复方案（Go 1.22+ 自动生效）**
>
> ```go
> // Go 1.22+ 无需额外修复！
> for _, v := range data {
>     go v.print() // 每次迭代 v 是独立的，输出 one two three
> }
> ```
>
> **指针切片场景（始终正确）**
...
> **总结**
>
> - **Go 1.22+** 修复了 `for`循环迭代变量的捕获问题，闭包默认捕获当前迭代的独立值。
> - **旧代码**：若依赖共享变量行为，需检查逻辑；其他情况可能自动变正确。
> - **最佳实践**：
>   - 新代码可直接捕获迭代变量。
>   - 显式使用 `v := v`或参数传递仍推荐，以增强代码可读性。
>   - 对性能敏感场景，可测试新旧版本的差异。
>
> 官方提案：[Go 1.22 Loop Variable Changes](https://github.com/golang/go/discussions/56010)
...
![img](https://segmentfault.com/img/remote/1460000013739014?w=480&h=134)
...
在 2012 年 Google I/O 大会上，Rob Pike 的 [Go Concurrency Patterns](https://link.segmentfault.com/?enc=%2F5SCs3Ltv7T10TkU39F4oQ%3D%3D.v3QR8rknNArmzvGjxlZ6D16Q9yRJAI6uFkINBAuDc%2FWzemHhCqe3OwIIkUKFAEULWL5X0feuLaG54Cf0TB%2BcsA%3D%3D) 演讲讨论 Go 的几种基本并发模式，如 [完整代码](https://link.segmentfault.com/?enc=3CryU0s5d%2F%2FfvvYrZPi6Iw%3D%3D.b%2B4cCEQlm3DIRodZ5p4fNOeOEAh%2FFeI1duTKcL%2F72yktbJrUBZOJRBHa1Xajb5QY4PsEIg1wthj8ySEB%2F1y%2FkpTajzIdeXJ4dFJg8wVuTcE%3D) 中从数据集中获取第一条数据的函数：

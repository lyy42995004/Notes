...
按照题意一开始想到的肯定是三层枚举＋集合去重，但时间和空间复杂度都太高了，所以要**转换思路**将三数之和化为两数之和（我自己没想出来）。
...
`l`和`r`不断缩小区间，通过`l_max`和`r_max`记录不断缩小时，左边和右边的最高值。当`height[l] < height[r]`，则**必有 `l_max < r_max`**，所以下标`l`处能接的雨水量等于`leftMax−height[left]`，另一种情况相同。